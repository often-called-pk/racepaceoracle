...
/**
 * Exports a DOM element (chart) as a PNG image with RacePaceOracle branding
 * @param elementRef - React ref to the DOM element to export
 * @param filename - Name for the downloaded file (without extension)
 */
export async function exportChartAsImage(
  elementRef: React.RefObject<HTMLElement>,
  filename: string = "racepaceoracle-chart"
): Promise<void> {
  ...
  try {
    ...
    const racePaceOracleText = document.createElement('span');
    racePaceOracleText.innerHTML = '<span style="color: white; font-weight: 600; letter-spacing: -0.02em;">Race</span><span style="color: #ef4444; font-weight: 600; letter-spacing: -0.02em;">PaceOracle</span>';
    brandingDiv.appendChild(gaugeIcon);
    brandingDiv.appendChild(racePaceOracleText);
