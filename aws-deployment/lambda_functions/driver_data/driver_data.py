# driver_data.py - Lambda function to handle driver data requests
import json
import boto3
import os
import logging
...
def lambda_handler(event, context):
    """
    Lambda function handler for driver data requests
    ...
    try:
        ...
        if driver_id:
            ...
            # Query for all drivers in the session
            response = table.query(
                IndexName='SessionIndex',
                KeyConditionExpression=Key('session_uid').eq(session_uid)
            )
        ...
        logger.error(f"Error retrieving driver data: {str(e)}")
        return {
            'statusCode': 500,
            'headers': get_cors_headers(),
